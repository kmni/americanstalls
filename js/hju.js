...
var hju	= {
		...
		if ($('.colorbox').length > 0)
		{
			...
			$('.colorbox').each(function(){
				...
				if (href.indexOf('#') > -1)
				{
					rel		= href.replace('#', '');
					title	= a.attr('title') || '';
					href	= $('.colorbox[rel="'+rel+'"]').eq(0).attr('href');
					// console.log(href);
					a.on('click', function(){
						$.colorbox({href:href, rel:rel, title:title, current:'{current} / {total}'});
						return false;
					});
				}
				else
	...
	colorOptions:	function(){
		if ($('.options_wrap a').length > 0 && $('img[class^="ribbon_"]').length > 0)
		{
			$('.options_wrap a').on('mouseover click', function(){
				var a		= $(this),
					index	= parseInt(a.attr('href').replace('#', ''));
				if ($('img.ribbon_'+index).length > 0)
				{
					$('img[class^="ribbon_"]').addClass('noscreen');
					$('img.ribbon_'+index).removeClass('noscreen');
				}
			});
			return false;
		}
